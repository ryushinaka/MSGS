...
{
    public class MiniScriptDataWarehouseEditor : OdinMenuEditorWindow
    {
        static Type[] types;
        static List<int> instanceids;

        [MenuItem("MiniScript/DataWarehouse Editor")]
        private static void OpenWindow()
        {
            GetWindow<MiniScriptDataWarehouseEditor>().Show();
        }
        ...
        protected override OdinMenuTree BuildMenuTree()
        {
            ...
            tree.AddAllAssetsAtPath("", "Assets", typeof(MiniScriptScriptableObject), true, true);

            var result = new OdinMenuTree();

            List<MiniScriptScriptableObject> list = new List<MiniScriptScriptableObject>();
            if (instanceids == null) { instanceids = new List<int>(); }
            else { instanceids.Clear(); } //clear the list before we build the menutree

            foreach (OdinMenuItem omi in tree)
            {
                if (omi.Value != null)
                {
                    var mso = (MiniScriptScriptableObject)omi.Value;

                    //if (mso.ScriptableObjectType == MiniScriptScriptableObjectType.EmbeddedValue)
                    //{
                    //    OdinMenuItem item = new OdinMenuItem(result, mso.name, mso);
                    //    result.Add(mso.name, mso);

                    //    if (mso.IsList() || mso.IsDictionary() && !instanceids.Contains(mso.GetInstanceID()))
                    //    {                            
                    //        AddChildren(ref item, ref mso);
                    //    }
                    //}
                }
            }
