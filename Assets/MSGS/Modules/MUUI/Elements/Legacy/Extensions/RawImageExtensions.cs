...
namespace MiniScript.MSGS.MUUI
{
    ...
    {
        public static ValMap ToValMap(this MiniScript.MSGS.MUUI.TwoDimensional.RawImage image)
        {
        ...
        public static bool UpdateImage(this MiniScript.MSGS.MUUI.TwoDimensional.RawImage image, Value a, Value b)
        {
            if (a is ValString)
            {
                ...
                switch (s)
                {
                    case "name":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify mouse event handler script for MUUI_Image(" + image.gameObject.name + ").name property with a non-String(ValString) parameter." +
                    ...
                    case "enabled":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify mouse event handler script for MUUI_Image(" + image.gameObject.name + ").enabled property with a non-boolean(ValNumber) parameter." +
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").alphaThreshold with a property label accessor that is not a Number. " +
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").fillAmount with a property label accessor that is not a Number. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").fillCenter with a property label accessor that is not a Boolean. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").fillClockwise with a property label accessor that is not a Boolean. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    ...
                    //            default:
                    //                MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").fillMethod property  with an argument that is not an approved string literal, no change made. " +
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").fillMethod property  with an argument that is not String, no change made. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    case "hasBorder":
                        #region
                        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").hasBorder is a read-only property, no change made. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                        return false;
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").preserveAspect with a property value that is not a Boolean. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    ...
                    //            default:
                    //                MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").type with a property value that is not an acceptable string literal. " +
                    ...
                    //    else
                    //    {
                    //        MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").type with a property value that is not a String. " +
                    //            " Argument given appears to be of type " + b.GetType().Name);
                    //        return false;
                    //    }
                    //#endregion
                    case "colorR":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").color.R with a property value that is not a Number. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                            return false;
                        }
                    #endregion
                    case "colorG":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").color.G with a property value that is not a Number. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                            return false;
                        }
                    #endregion
                    case "colorB":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").color.B with a property value that is not a Number. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                            return false;
                        }
                    #endregion
                    case "colorA":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").color.A with a property value that is not a Number. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                            return false;
                        }
                    #endregion
                    case "maskable":
                        ...
                        {
                            MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ").maskable with a property value that is not a Boolean. " +
                                " Argument given appears to be of type " + b.GetType().Name);
                            return false;
                        }
                    #endregion
                    default:
                        if (b is null) { MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ") with an unknown property label (" + s + ")."); }
                        else { MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ") with an unknown property label (" + s + ") with an argument that appears to be of type " + b.GetType().ToString()); }
                        return false;
                }
            }
            else
            {
                if (a == null)
                {
                    MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ") with a property label accessor that is null.");
                }
                else if (a != null)
                {
                    MiniScriptSingleton.LogError("Attempt to modify MUUI_Image(" + image.gameObject.name + ") with a property label accessor that is not a String. It appears to be a (" + a.GetType().Name + ").");
                }
        ...
        public static void SetupImage(this MiniScript.MSGS.MUUI.TwoDimensional.RawImage image, ref DataRow row)
        {
