...
namespace MiniScript.MSGS.MUUI.TwoDimensional
{
    ...
    public class MUUIImageAnimated : MonoBehaviour, UI_Element, IControl, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        float mscroll;
        bool benter;
        ...
        void Start()
        {
            ...
            eventFilter = new ControlCollectionEventFilter()
            {
                events = new List<int>()
                {
                    ImageAnimatedMouseEnter,
                    ImageAnimatedMouseExit,
                    ImageAnimatedMouseLeftClick,
                    ImageAnimatedMouseRightClick,
                    ImageAnimatedMouseMiddleClick,
                    ImageAnimatedMouseDoubleLeftClick,
                    ImageAnimatedMouseDoubleRightClick,
                    ImageAnimatedMouseScrollUp,
                    ImageAnimatedMouseScrollDown,
                }
            };
        ...
        private IEnumerator AnimationCycle()
        {
            while (true)
            {
                ...
                if (DoAnimate && sprites.Count > 0)
                {
                    ...
                    this.GetComponent<MUUIImage>().image.sprite = sprites[currentFrame];
                }
            }
        }
        ...
        private static void EditorAddImage()
        {
            ...
            if (go == null) { return; }
            go.AddComponent<MiniScript.MSGS.MUUI.TwoDimensional.MUUIImageAnimated>();
            //make sure the references are put in place
            go.GetComponent<MiniScript.MSGS.MUUI.TwoDimensional.MUUIImageAnimated>().image = go.GetComponent<UnityEngine.UI.Image>();
            go.GetComponent<MiniScript.MSGS.MUUI.TwoDimensional.MUUIImageAnimated>().Rect = go.GetComponent<UnityEngine.RectTransform>();
