...
namespace MiniScript.MSGS.MUUI.TwoDimensional
{
    ...
    public class Image : MonoBehaviour, UI_Element, IControl, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler
    {
        float mscroll;
        bool benter;
        ...
        private static void EditorAddImage()
        {
            ...
            if (go == null) { return; }
            go.AddComponent<MiniScript.MSGS.MUUI.TwoDimensional.Image>();
            //make sure the references are put in place
            go.GetComponent<MiniScript.MSGS.MUUI.TwoDimensional.Image>().image = go.GetComponent<UnityEngine.UI.Image>();
            go.GetComponent<MiniScript.MSGS.MUUI.TwoDimensional.Image>().Rect = go.GetComponent<UnityEngine.RectTransform>();
